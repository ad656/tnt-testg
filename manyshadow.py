#Changes to base.py in order to process commands from the shadow api. 
# - A D
...
class logger:
    ...
    def getDeviceHealth(self):
        url = "https://api.staging.tagntrac.io/device/{0}/health".format(self.addr)
        headers = {
    ...
    def updateBLEGatewayConfig(self,gwId, cmd):
        global runs
        url = "https://api.staging.tagntrac.io/device/{0}/shadow".format(gwId)
        headers = {
...
if __name__=="__main__":
    ...
    for i in range(0, len(addr_list), segments):
        addlist = addr_list[i:i+segments]
...
##                 'C46A7CF90016','C46A7CF90017']
    ...
    for index in range(list_size):
        ...
        #dev = addr_list[index]
        this = alist[int(index/segments)]
        print(f"Currently sending: {this}")
        dev = alist[int(index/segments)][index%segments]
